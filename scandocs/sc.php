...
foreach (new DirectoryIterator($directory) as $YearInfo) {
	...
	foreach (new DirectoryIterator($directory.'/'.$Y) as $CadnumInfo) {
		...
		$sql = "INSERT INTO docs2 SET par_id='$year_id', name='{$CadnumInfo->getFilename()}', cyear='$Y', tp=1, cdate=str_to_date('".date("Y-m-d",$CadnumInfo->getMTime())."', '%Y-%m-%d')";
		$db->query($sql);
		//echo $sql."<br>";
		...
		foreach (new DirectoryIterator( $CadnumPath  ) as $VolInfo) {
			...
			$sql = "INSERT INTO docs2 SET par_id='$cadnum_id', name='{$VolInfo->getFilename()}', cyear='$Y', tp=2, cdate=str_to_date('".date("Y-m-d",$VolInfo->getMTime())."', '%Y-%m-%d')";
			$db->query($sql);
			//echo $sql."<br>";
			...
			foreach (new DirectoryIterator( $VolPath  ) as $FileInfo) {
				...
				if( $FileInfo->getExtension()=='pdf' || $FileInfo->getExtension()=='sig'){
					...
					$sql = "INSERT INTO docs2 SET par_id='$vol_id', name='{$FileInfo->getFilename()}', cyear='$Y',tp=$tp, cnt_size={$FileInfo->getSize()}, cdate=str_to_date('".date("Y-m-d",$FileInfo->getMTime())."', '%Y-%m-%d')";
					$db->query($sql);
					//echo "-".$sql."<br>";

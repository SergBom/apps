...
$domain = $_POST['domain']; //<>'локально' ) ? $_POST['domain'] : ''; 
...
$result = array();
/*$sql = "select u.*, d.name domain from `prt#users` u 
...
if ($domain == 'локально'){ //====== Пользователь локальный
	...
	if ($resultDb = $db->query("select * from `prt#v#users` WHERE userName='$userName' and domain='$domain'")) {
	...
	if ($ad_conn['ds']) {
		...
		if ( $ldapBindUser ){ // Авторизация прошла успешно
			...
			if( $ent['count'] > 0) { //**** Пользователь найден в LDAP
				...
				if ($resDb = $db->query("select * from `prt#v#users_full` WHERE username='$userName' and org_id='{$ad_conn['domain_id']}'")) {
...
//echo "<Нашел запись><br>";		
		...
		//				echo "$sql<br>";
						...
						if($db->getOne("SELECT count(*) FROM `prt#users_group` WHERE user_id={$user_id} AND group_id=2")==0){
							$db->query("INSERT `prt#users_group` SET user_id={$user_id}, group_id=2");
						}
						// Добавляем основную группу
						if($db->getOne("SELECT count(*) FROM `prt#users_group` WHERE user_id={$user_id} AND group_id=3")==0){
							$db->query("INSERT `prt#users_group` SET user_id={$user_id}, group_id=3");
						}
						// Добавляем группу Редакторы главной страницы
						if($db->getOne("SELECT count(*) FROM `prt#users_group` WHERE user_id={$user_id} AND group_id=4")==0){
...
if($result['success']){
	...
	$db->query("UPDATE `prt#users` SET lastvisitDate='$now' WHERE id=$user_id");
}

...
////////////////////////////////////////////////////////////////////////		
		...
		$sql = "SELECT id,username,dateOff,dateOn,ad_state,off FROM `prt#users` WHERE domain_id='$domain_id' AND deleted=0";
		if ( $result = $dbo->query($sql) ){
			while ($row = $result->fetch()) {
				...
				if( !$sr ){ // Удаляем, если пользователя в домене не существует
					echo " ---> <font color='red'>Удаляем</font><br>";
					echo " | >> prt#users_ad_group";
						//$dbo->query("DELETE FROM `prt#users_ad_group`	WHERE user_id={$row['id']}");
					echo " - Delete<br>";
					echo " | >> prt#users_group";
						//$dbo->query("UPDATE `prt#users_group`  SET deleted=1, e_date=NOW() 		WHERE user_id={$row['id']}");
					echo " - Delete<br>";
					echo " | >> prt#users";
						//$dbo->query("UPDATE `prt#users` SET deleted=1, e_date=NOW(), dateOff=NOW() WHERE id={$row['id']}");
					echo " - Delete<br>";
					
				}else{
					echo " ---> <font color='blue'>Оставляем</font><br>";
					/////////////////////////////////////////////////////////
					/// now - сегодня
					/// ----------------
					...
					echo " OFF='".$row['dateOff']."' ON='".$row['dateOn']."' dOFF='$dOFF' dON='$dON'<br>";
					...
					if($row['dateOff']){
						if( $dOFF <= 0 ){
							if($row['dateOn']){
								if( $dON > 0 ){
									ad_userState($ad_conn,$ent,0);
									echo " ---> <font color='red'>Выключить1a</font><br>";
								} else {
									ad_userState($ad_conn,$ent,1);
									echo " ---> <font color='green'>Включить1a</font><br>";
								}
							} else {
								ad_userState($ad_conn,$ent,0);
								echo " ---> <font color='red'>Выключить1b</font><br>";
							}
						} else {
							echo "???";
							// dateOff еще не наступила
							ad_userState($ad_conn,$ent,1);
							echo " ---> <font color='green'>Включить1к</font><br>";
						}
					} else {
						if($row['dateOn']){
							if( $dON > 0 ){
								ad_userState($ad_conn,$ent,0);
								echo " ---> <font color='red'>Выключить1c</font><br>";
							} else {
								ad_userState($ad_conn,$ent,1);
								echo " ---> <font color='green'>Включить1c</font><br>";
							}
						} else {
							ad_userState($ad_conn,$ent,1);
							echo " ---> <font color='green'>Включить1d</font><br>";
						}
					}
...
//////////////////////////////////////////////////////
function ad_userState($ad_conn,$ent,$_enable=1){
...
*/
	...
	$userdata["useraccountcontrol"][0]=$newState;
	ldap_modify($ad_conn['ds'], $dn, $userdata);

	$sr = ldap_search($ad_conn['ds'], $ldapBase, "(samaccountname=$sa)");
	$ent= ldap_get_entries($ad_conn['ds'],$sr);

...
Ext.Loader.setPath('Ext.ux', '/ext60/build/ux');
...
//        autoLoad: true,
        ...
        proxy: {
            type: 'rest',
            url: 'tir-errorDescription.LIST.php',
            extraParams:{
...
/***********************************************************************/
    ...
    function cError(val) {
        if (val == 0 || val == null) {
            return '<img src="/resources/images/diagona/02/10/21.png" alt="Bad">';
        } else if (val ==1) {
            return '<img src="/resources/images/diagona/02/10/22.png" alt="Ok">';
        } else if (val ==2) {
            return '<img src="/resources/images/diagona/02/10/24.png" alt="Not">';
        } else if (val ==3) {
            return '<img src="/resources/images/diagona/02/10/23.png" alt="No Err">';
        } else if (val ==4) {
            return '<img src="/resources/images/flag/png/ru.png">';
        }
        return val;
    }
/***********/
...
/***********/
	...
	//> prop_type=3     -------------Муниципальные образования (40 штук)
			...
			return '<img src="/resources/images/flag/png/ru_2.png">Мур';
		} else if (val == 3) {
			return '<img src="/resources/images/flag/png/ru_2.png">МО';
		} else {
			return '<img src="/resources/images/flag/png/ru_2.png">'+val;
		}
	}
/************ FILTERS *********/
...
/***********************************************************************/
    var gridListErrors = Ext.create('Ext.grid.Panel',{
       ...
       columns: [
                     ...
                     {text: 'КЛАДР', dataIndex: 'kladr_ny', width: 20 },
...
//                        comboErrors.select( records[0].get('status') );
    ...
    var ErrorForm = Ext.create('Ext.form.Panel', { //Ext.widget('form',{ 
        ...
        items: [{
             ...
             labelWidth: 250,
			 width: 300,
			 fieldLabel: 'Количество записей с данным External_ID',
             name: 'countExtID',
             id: 'countExtID',
			 readOnly: true
        },{
...
//					xtype: 'hidden' 
        ...
        dockedItems: [{
            ...
            items: [{
                    ...
                    if (Ext.getCmp('rec_guid').getValue()) {
                        ...
                        Ext.Ajax.request({
                             ...
                             success: function(response){
                                 loadMask.hide();
                                 Ext.create('Ext.window.Window', {
                                      title: '<img src="/resources/images/flag/png/ru.png"> Подробности по Сущности из ССД!',
                                      height: 200,
                                      width: 400,
                                      stateful: true,
                                      stateId: 'winDetailRF',
                ...
                text: '',
                icon: '/resources/images/silk_icons/icons/chart_organisation.png',
               
                handler: function(){
					var form = this.up('form').getForm();
                    if (Ext.getCmp('essence_type').getValue() == 'Субъект') {
						var ExtID = Ext.getCmp('external_id').getValue(); //  parseInt( , 10 );
						ExtID = ExtID.replace(/\D/g, '');
//						alert('Subject '+ExtID);

                        var loadMask = new Ext.LoadMask(Ext.getBody(), {msg:"Loading..."});
                        loadMask.show();
                        Ext.Ajax.request({
                             url: 'tir-treeSubjIdNoAddr.php',
                             params: {
                                 ext_id: ExtID
                             },
                             success: function(response){
                                 loadMask.hide();
                                 Ext.create('Ext.window.Window', {
                                      title: '<img src="/resources/images/silk_icons/icons/chart_organisation.png"> Список ID без Адреса',
                                      height: 200,
                                      width: 400,
                                      stateful: true,
                                      stateId: 'winTreeSubjNoAddr',
                                      autoScroll: true,
                                      items: {
                                          html: response.responseText
                                      }
    ...
    if(adm){
       var temp = Ext.getCmp('kladr_yes'); temp.readOnly = false;
    }else{
       var temp = Ext.getCmp('kladr_yes'); temp.readOnly = true;
    }
    ...
    if(!win){
           win = Ext.create('widget.window', {
                ...
                tbar: [
					   ...
					   {
							xtype: 'button',
							text: '<font color="red"><b>К ошибкам ФНС</b></font>',
							handler : function() {
								window.location = "/index.php?option=com_content&view=article&id=47&Itemid=217"
							}
					   }
               ]
            });

...
if($ps->t == 1){ // Детали ошибки ТИР

					$connTIR = ConnectTIR(); // Присоска к базе

	$sql1 = "select 
		replace(replace(replace(to_char(SQL$),':external_id','''".$ps->external_id."''')
	                ,':doc_guid','''".$ps->doc_guid."''')
			,':normalized_number','45') as F1
		FROM T\$FNS#SQL
		WHERE SQL_TYPE=3 
			AND PATH= Upper('".$ps->path."')";
	//echo $sql1;
		
	$stid = oci_parse($connTIR, $sql1);
	oci_execute($stid);
	$row1 = oci_fetch_array($stid);


	$stid = oci_parse($connTIR, $row1['F1']);
	oci_execute($stid);

	$nrows = oci_fetch_all($stid, $d);
...
//*****************************************************************************//
}else if($ps->t == 2 ){ // Детали ошибки РФ

	$connSSD = ConnectOciDB('EGRP'); // Присоска к базе
$sql = "";
switch ($ps->essence_type){
	case "Субъект":
...
WHERE ee.id= to_number(regexp_substr( '{$ps->external_id}', '([0123456789])+')) ";
			break;
	case "Объект":
...
//					ro.RE_INCLUDE_ID    \"Ссылка на объект, включающий данный (Земельный участок для здания, здание для помещения)\",
			$sql = "SELECT
				...
				WHERE ro.id= to_number(regexp_substr( '{$ps->external_id}', '([0123456789])+'))";
			break;
	case "Право":
			$sql = "SELECT  
				rt.ID,
				...
				WHERE rt.id= to_number(regexp_substr( '{$ps->external_id}', '([0123456789])+'))";
			break;
	case "Обременение":
			break;
}
